fetch("./data.json")
    ...
    .then(myAllProducts => loadAllProducts(myAllProducts));
function loadAllProducts(myAllProducts) {
    console.log(myAllProducts);
    var mainContainer = document.getElementById("row-product-all");
    console.log(mainContainer);
    let products = myAllProducts;
    for (var category in products) {
        if (products.hasOwnProperty(category)) {
            ...
            for (var i = 0; i < products[category].length; i++) {
                var product = products[category][i];
                let div = document.createElement("div");
        div.innerHTML = `<br>
        <div class="col">
                        <div class="card" style="box-shadow: none; border: 0cap;">
                            <img class='seeds-image' src=${product.url} alt=${product.description}>
                            <div class="card-body">
